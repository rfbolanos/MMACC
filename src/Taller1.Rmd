...
p_load(tidyverse,kableExtra,glmnet,data.table,car,pracma,knitr)
...
Ajuste la regresión ridge y lasso con los valores estimados de $\lambda_r$ y $\lambda_l$
...
df<-data.frame(Modelos=rep(c("Ridge","Lasso"),each= 2),Lambdas=c("Min","Se1"),
...
df[3,4]<- cell_spec(df[3,4], bold = T, color = "green" )
...
En ese sentido l modelo que tuvo menor error cuadratico medio es el modelo lasso con el $\lambda_{min}$. 
...
Modelo<-glmnet(y=taller1_200$y,x= taller1_200[,-1],alpha = 1,lambda =lasso_models$lambda.min )
