...
predicciones <- predict(ridge_model_min, as.matrix(taller1_200[, -1]))

# Calcular el error cuadrático medio
ecm_ridge_min <- mean((predicciones - taller1_200$y)^2)

#No. 2 
# Realizar predicciones en los datos de prueba
predicciones <- predict(ridge_model_1se, as.matrix(taller1_200[, -1]))

# Calcular el error cuadrático medio
ecm_ridge_1se <- mean((predicciones - taller1_200$y)^2)

#No. 3 
# Realizar predicciones en los datos de prueba
predicciones <- predict(lasso_model_min, as.matrix(taller1_200[, -1]))

# Calcular el error cuadrático medio
ecm_lasso_min <- mean((predicciones - taller1_200$y)^2)

#No. 4 
# Realizar predicciones en los datos de prueba
predicciones <- predict(lasso_model_1se, as.matrix(taller1_200[, -1]))

# Calcular el error cuadrático medio
ecm_lasso_1se <- mean((predicciones - taller1_200$y)^2)



data.frame(Modelos= c("Ridge","Lasso"),
            Lambdamin=c(ridge_models$lambda.min,lasso_models$lambda.min),
            ECMLambdamin= c(ecm_ridge_min, ecm_lasso_min), 
            Lambda1se =c(ridge_models$lambda.1se,lasso_models$lambda.1se),
            ECMLambda1se=c(ecm_ridge_1se, ecm_lasso_1se)) %>% 
  knitr::kable(booktabs = FALSE,align="c") %>% kable_styling(position = "center")
